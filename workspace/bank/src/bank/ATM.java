...
//			내일 강사님께 질문
			...
			while(true) {
				...
				if(choice == 1) { // 계좌 개설
					arBank = new Bank[] {new Shinhan(), new Kookmin(), new KaKao()}; // 규칙성을 만들기 위해.
					...
					user = arBank[index];
					account = index + account; // 무슨 은행인지 알기 위해서 index를 넣어서 0번이면 신한.. 이렇게 간다.
					...
					if(user != null) {
						...
						if(user.getPassword().equals(password)) {
							...
							account = user.getAccount().charAt(0)/*원래 가지고 있던 은행번호*/ + account;
							user.setAccount(account);
						}
					}else {
				...
				switch(choice) {
				case 2: // 입금하기(계좌를 개설한 은행에서만 입금가능. 힌트 : '계좌번호에 무슨 은행인지 표시')
					if(user != null) {
						if(user.getAccount().startsWith(index + "")) { // 계좌번호의 시작이 인덱스니?
