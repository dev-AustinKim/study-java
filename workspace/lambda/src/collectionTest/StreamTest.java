...
// hash는 키와 value가 합쳐진 것이기 때문에 앞에 key는 set타입이고 value는 collection 타입이다.
// 컬렉션 타입이면 stream을 무조건 쓸 수 있다.
...
//	      forEach() : Stream에서 바로 쓸 수 있는건 forEach밖에 없다. 나머지는 사용하려면 Stream을 통해서 들어가야한다. (ex : steam.map())
...
//		IntStream.rangeClosed(1, 10).filter(n->streamTest::check).forEach(n -> System.out.print(n));
...
//		datas.stream()를 해줌으로써 arrayList 타입인 datas를 stream타입으로 바꿔준 것이다.
